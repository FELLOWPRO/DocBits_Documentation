...
Bloki `ApplicableTradeTax` są rozdzielane na trzy poziomy oparte na stawce (nie pozycyjnie `[1]`/`[2]`/`[3]`): pola stawki standardowej (`TAX_RATE` / `NET_AMOUNT` / `TAX_AMOUNT`) obejmują stawkę ≥ 19; pola stawki obniżonej (`*_2`) obejmują 0 < stawka < 19; pola stawki zerowej (`*_3`) obejmują stawkę = 0. Pełną listę pól znajdziesz w [Podział podatków ZUGFeRD](../README.md#tax-breakdown-tier-classified).

| Ścieżka ZUGFeRD CII | Pole DocBits | Pole Infor BOD | Typ | Opis |
| :--- | :--- | :--- | :--- | :--- |
| `ApplicableTradeTax/RateApplicablePercent` (poziom 1) | `TAX_RATE` | `TaxPercent` | NUMBER | Standardowa stawka VAT (≥ 19) |
