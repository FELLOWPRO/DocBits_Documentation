---
description: Wie Sie einen Receive Delivery BOD manuell über die API in DocBits importieren
---

# Wareneingänge importieren (Receive Delivery BOD)

Wareneingänge erreichen DocBits normalerweise automatisch über Ihren ION-Datenfluss. Diese Seite beschreibt, wie Sie einen **Receive Delivery BOD** manuell senden — nützlich, wenn Sie einen Wareneingang erneut importieren, einen Stapel nachladen möchten, der nie angekommen ist, oder eine Feldzuordnung testen wollen, bevor Sie den automatischen Datenfluss einschalten.
...
| --- | --- |
| `/import/receive_delivery_bod` | Sie haben den BOD als **XML-Datei** und möchten sie hochladen. |
| `/import/receive_delivery_bod_xml` | Sie möchten den **XML-Inhalt** im Request senden statt einer Datei. Der BOD muss in JSON verpackt werden, daher eignet sich das für kurzes XML oder für ein anderes System, das die API aufruft — für einen vollständigen BOD von Hand laden Sie besser die Datei hoch. |
...
* **Den BOD** — eine `SyncReceiveDelivery`-XML-Datei oder deren Inhalt.
...
* [Sandbox API (Vereinigte Staaten)](https://us.sandbox.api.docbits.com/docs#/import/import_receive_delivery_bod_import_receive_delivery_bod_post)
* [Production API (Europa)](https://eu.api.docbits.com/docs#/import/import_receive_delivery_bod_import_receive_delivery_bod_post)
* [Production API (Vereinigte Staaten)](https://us.api.docbits.com/docs#/import/import_receive_delivery_bod_import_receive_delivery_bod_post)
...
#### Eine Datei hochladen — `/import/receive_delivery_bod`

![](https://raw.githubusercontent.com/Fellow-Consulting-AG/docbits/refs/heads/main/readme/.gitbook/assets/import-rd-form.png)

| Feld | |
| --- | --- |
| **file** | Erforderlich. Klicken Sie auf **Choose file** und wählen Sie Ihre `SyncReceiveDelivery`-XML-Datei aus. |
...
| **custom\_fields\_mapping** | Optional. Zusätzliche Kopffelder. Siehe [Benutzerdefinierte Feldzuordnungen](#benutzerdefinierte-feldzuordnungen) unten. |
| **custom\_line\_fields\_mapping** | Optional. Dasselbe für zusätzliche Felder auf den Wareneingangszeilen. |
| **populate\_additional\_info** | Optional, standardmäßig `false`. Setzen Sie es auf `true`, damit DocBits nach dem Import zusätzliche Wareneingangsinformationen aus dem ERP nachlädt. Lassen Sie es auf `false`, sofern Sie es nicht sicher brauchen — es macht den Import langsamer. |
...
Zeilenzuordnungen verwenden dieselben Namen `custom_field_1` … `custom_field_10`, ihre XPaths werden jedoch **relativ zu jeder Wareneingangszeile** gelesen und beginnen deshalb mit `./`:

```json
{"custom_field_1": "./UserArea/Property/NameValue[@name='User defined 1']/text()"}
```

#### Das XML einfügen — `/import/receive_delivery_bod_xml`
...
Dieser Endpunkt nimmt `org_id`, `sub_org_id` und `populate_additional_info` entgegen, aber **überhaupt keine Feldzuordnungen** — weder Kopf- noch Zeilenzuordnungen. Wenn Ihre Wareneingänge benutzerdefinierte Felder brauchen, laden Sie stattdessen die Datei hoch.
...
Receive Delivery BODs werden verarbeitet, während Sie warten — wenn Sie diese Meldung sehen, sind die Daten also bereits da.

Wenn am Request etwas nicht stimmte, erhalten Sie `"success": false` zusammen mit einer Meldung, die das Problem beschreibt.

### 5. Prüfen, ob die Daten angekommen sind

* Gehen Sie in DocBits zu **Settings → Document Processing → Lookup Master Data**.
* Wählen Sie links **BOD Input Data** und öffnen Sie dann den Tab für die importierten Daten.
* Suchen Sie nach dem Wareneingang oder der Bestellung, zu der er gehört.

<!-- SCREENSHOT: Lookup Master Data with BOD Input Data selected and the goods receipt data shown -->

{% hint style="info" %}
DocBits entscheidet anhand des Typs *innerhalb* des BOD, was damit geschieht — nicht anhand des verwendeten Import-Endpunkts. Wenn Sie hier versehentlich einen anderen BOD senden, wird er als dieser Typ importiert statt abgelehnt — prüfen Sie also, ob der Tab, in dem Sie die Daten finden, der erwartete ist.
{% endhint %}
