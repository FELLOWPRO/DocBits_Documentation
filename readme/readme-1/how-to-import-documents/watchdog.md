...
Kada se otvori, pozovite fajl tako što ćete uneti putanju gde se fajl WatchDog.exe nalazi na vašem uređaju kao što je prikazano ispod
...
Unesite "install" nakon putanje fajla i pritisnite ENTER na tastaturi
...
Treba da dobijete sledeću grešku jer će pokrenuti aplikaciju u UI režimu i još uvek nema konfiguracije.
...
Aplikacija će se automatski otvoriti nakon instalacije. Na vrhu, idite na Einstellungen (Podešavanja) i ovde ćete videti 2 odvojene sekcije.

**Podešavanja foldera (Ordner Einstellungen)**
...
Ovde ćete konfigurisati putanje fajlova gde će dokumenti biti čitani i obrađivani. Putanja za čitanje će biti za dokumente koje želite da WatchDog izvuče i obradi, dok će obrađena putanja biti korišćena da pokaže koje dokumente iz čitalačkog foldera su izvučeni i obrađeni.

**Opšta podešavanja (Allgemeine Einstellungen)**
...
Ovde ćete konfigurisati vašu aplikaciju DocBits sa WatchDog-om. Bićete obavezni da unesete svoje DocBits okruženje, API ključ i tip dokumenta (kao što je naznačeno iznad). Da biste dodali više od jednog tipa dokumenta, odvojite imena tipova dokumenata zarezom (RAČUN,OTPREMNICA,NARUDŽBENICA, itd.)

Ako je uneta nevažeća konfiguracija za bilo koje od polja, dobićete grešku kao u primeru ispod.
...
Kada završite, vratite se na područje Command Prompt-a i ponovo pokrenite fajl WatchDog.exe i ovog puta će biti ispravno instaliran zbog sada prisutnih konfiguracija
...
Da biste pokrenuli aplikaciju u UI režimu, pokrenite sledeću komandu
...
Ako želite samo da pokrenete aplikaciju, umesto toga pokrenite ovu komandu.
...
Da biste postavili WatchDog aplikaciju na način koji će joj omogućiti automatsko pokretanje kada uključite sistem.

Jednostavno otvorite "Servise" u podešavanjima vašeg Windows uređaja.
...
Potražite WatchDog u lokalnim servisima i otvorite servis (dvoklik na unos)
...
U gornjem meniju imaćete sledeće opcije za automatsko pokretanje:

* Izaberite Automatski, servis će se pokrenuti prilikom podizanja sistema
* Automatski (Odgodjeni start), sačekaće 2 minuta nakon podizanja sistema da pokrene servis
